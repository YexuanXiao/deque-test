...
int main(int, char**) {
  static_assert(test_constraints_insert_range<bizwen::deque, int, double>());

  for_all_iterators_and_allocators<int, const int*>([]<class Iter, class Sent, class Alloc>() {
    test_sequence_insert_range<bizwen::deque<int, Alloc>, Iter, Sent>([]([[maybe_unused]] auto&& c) {
      LIBCPP_ASSERT(c.__invariants());
    });
  });
  test_sequence_insert_range_move_only<bizwen::deque>();

  test_insert_range_exception_safety_throwing_copy<bizwen::deque>();
  test_insert_range_exception_safety_throwing_allocator<bizwen::deque, int>();
