...
void test() {
  typedef bizwen::deque<T, Allocator> C;
...
int main(int, char**) {
  ...
  static_assert((std::is_same<bizwen::deque<char>::allocator_type, std::allocator<char> >::value), "");

#if TEST_STD_VER >= 11
  {
    typedef bizwen::deque<short, min_allocator<short>> C;
