...
int main(int, char**) {
  {
    typedef bizwen::deque<int> C;
    C c = make<bizwen::deque<int> >(10);
  ...
  {
    typedef bizwen::deque<int> C;
    const C c = make<bizwen::deque<int> >(10);
...
#if TEST_STD_VER >= 11
  {
    typedef bizwen::deque<int, min_allocator<int>> C;
    C c = make<bizwen::deque<int, min_allocator<int>> >(10);
    ASSERT_SAME_TYPE(decltype(c[0]), C::reference);
    LIBCPP_ASSERT_NOEXCEPT(c[0]);
    LIBCPP_ASSERT_NOEXCEPT(c.front());
    ASSERT_SAME_TYPE(decltype(c.front()), C::reference);
  ...
  {
    typedef bizwen::deque<int, min_allocator<int>> C;
    const C c = make<bizwen::deque<int, min_allocator<int>> >(10);
    ASSERT_SAME_TYPE(decltype(c[0]), C::const_reference);
    LIBCPP_ASSERT_NOEXCEPT(c[0]);
    LIBCPP_ASSERT_NOEXCEPT(c.front());
    ASSERT_SAME_TYPE(decltype(c.front()), C::const_reference);
